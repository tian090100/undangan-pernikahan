document.addEventListener('DOMContentLoaded', () => {
  ...
  const target = new Date('2027-10-10T08:00:00+07:00').getTime();
  ...
  const calendarParams = new URLSearchParams({
    action: 'TEMPLATE',
    text: 'Pernikahan Arga & Laras',
    dates: '20271010T010000Z/20271010T060000Z',
    location: 'Pendopo Agung Royal Ambarrukmo, Yogyakarta',
    details: 'Pawiwahan Arga Pradana Wicaksana dan Larasati Ayu Kinasih.'
  });
  ...
  let savedWishes = [];
  try { savedWishes = JSON.parse(localStorage.getItem('daprita-jawa-premium-wishes')) || []; } catch {}
  const sampleWishes = [
    { name: 'Keluarga Wicaksana', message: 'Selamat menempuh hidup baru. Semoga senantiasa rukun, bahagia, dan penuh berkah.' },
    { name: 'Dimas & Ayu', message: 'Turut berbahagia. Semoga cinta kalian tumbuh semakin indah setiap hari.' }
  ];
  const renderWishes = () => {
    const list = document.getElementById('wish-list');
    list.replaceChildren();
    [...savedWishes, ...sampleWishes].slice(0, 6).forEach((wish) => {
  ...
  document.getElementById('rsvp-form').addEventListener('submit', (event) => {
    ...
    try { localStorage.setItem('daprita-jawa-premium-wishes', JSON.stringify(savedWishes)); } catch {}
    renderWishes();
    document.getElementById('form-status').textContent = `Matur nuwun, ${name}. Konfirmasi Anda tercatat pada demo ini.`;
